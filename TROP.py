...
def line_search(actor, get_loss, get_kl, prev_params, fullstep, expected_improve, max_backtracks=10, delta=0.01):
    original_params = get_flat_params_from(actor)  # 保存原始参数
    original_loss = get_loss().item()              # 原始损失值
    original_kl = get_kl().item()                 # 原始KL散度
    
    for step_frac in [0.5**i for i in range(max_backtracks)]:  # 尝试步长衰减序列（1.0, 0.5, 0.25...）
        new_params = prev_params + step_frac * fullstep        # 候选参数
        set_flat_params_to(actor, new_params)                 # 临时设置新参数
        
        # 计算新参数下的损失和KL
        current_loss = get_loss().item()
        current_kl = get_kl().item()
        
        # 恢复原始参数，避免影响后续计算
        set_flat_params_to(actor, original_params)
        
        # 检查条件：KL约束 且 损失改进足够
        if current_kl <= delta and current_loss <= original_loss + 1e-4 * step_frac * expected_improve:
            return True, new_params  # 找到可行步长
...
def run_trpo(seed=0):
    ...
    while total_steps < max_steps:
        ...
        success, new_params = line_search(
            ...
            fullstep=fullstep,
            expected_improve=expected_improve,
            delta=0.01  # 根据超参数设定
        )
