...
def run_ppo_with_net(seed=0):
    ...
    while total_steps < max_steps:
        ...
        states_tensor = states  # already on device
        with torch.no_grad():
        ...
        for _ in range(K_epoch):
            mb_probs = actor(states)
            mb_dist = torch.distributions.Categorical(mb_probs)
            mb_log_probs = mb_dist.log_prob(actions)

            mb_ratio = torch.exp(mb_log_probs - log_probs_old_tensor)
            mb_surr1 = mb_ratio * advantages
            mb_surr2 = torch.clamp(mb_ratio, 1 - clip_ratio, 1 + clip_ratio) * advantages
            ...
            mb_value_preds = critic(states).squeeze()
            mb_value_loss = F.mse_loss(mb_value_preds, returns)
...
if __name__ == "__main__":
    ...
    df_eval.to_csv('./results/ppo_k_epoch_score.csv', index=False)
    ...
    df.to_csv('./results/ppo_k_epoch_results.csv', index=False)
